...
module.exports = {
  ...
  publicKeyNegate: function (output, pubkey) {
    try {
      output.set(Point.fromHex(pubkey).negate().toRawBytes(isCompressedPublicKey(output)))
    } catch (e) {
      return 1
    }
    return 0
  ...
  publicKeyCombine: function (output, pubkeys) {
    try {
      let point = Point.fromHex(pubkeys[0])
      for (let i = 1; i < pubkeys.length; ++i) {
        point = point.add(Point.fromHex(pubkeys[i]))
      }
      try {
