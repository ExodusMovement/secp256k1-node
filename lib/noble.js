...
module.exports = {
  ...
  ecdsaRecover: function (output, sig, recid, msg32) {
    try {
      const res = noble.recoverPublicKey(msg32, sig, recid, isCompressedPublicKey(output));
      // if(!res) return 2
      output.set(res)
    } catch (e) {
      console.warn('ecdsaRecover', e)
      if(e.message === "Invalid Signature: r must be 0 < r < n")return 2
      if(e.message === "Invalid Signature: s must be 0 < s < n")return 2
      // if(e.message.includes("Invalid Signature")) return 1
      return 1
    }
    return 0
  },
